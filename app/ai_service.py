...
def provider_status() -> list[dict[str, Any]]:
    return [
        {"provider": "gemini", "configured": bool(settings.gemini_api_key), "model": settings.gemini_model},
        {"provider": "groq", "configured": bool(settings.groq_api_key), "model": settings.groq_model},
        {"provider": "openrouter", "configured": bool(settings.openrouter_api_key), "model": settings.openrouter_model},
        {"provider": "openai-legacy", "configured": bool(settings.openai_api_key), "model": settings.openai_model},
    ]
...
def _messages(user_prompt: str, data_context: str, analysis_type: str | None, workspace: str | None, filters: dict[str, Any] | None) -> list[dict[str, str]]:
    context = "\n".join([
        "DATA_CONTEXT_START", data_context or "No analytical data context was supplied.", "DATA_CONTEXT_END",
        f"Workspace: {workspace or 'Not specified'}", f"Analysis type: {analysis_type or 'Not specified'}", f"Active filters: {filters or 'None'}",
    ])
...
async def generate_ai_response(user_prompt: str, data_context: str = "", analysis_type: str | None = None, workspace: str | None = None, filters: dict[str, Any] | None = None) -> AIResponse:
    ...
    for index, (provider, call) in enumerate(providers):
        try:
            ...
            log.info("AI provider succeeded provider=%s model=%s fallback=%s latency_ms=%s", provider, model, index > 0, round((time.perf_counter() - started) * 1000))
            ...
            log.exception("AI provider failed provider=%s detail=%s", provider, detail)
    raise AIUnavailable("all configured AI providers failed", failures)
